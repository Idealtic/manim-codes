...
class vd7 (Scene):
    def construct(self):
        ...
        t1 = Tex(r"Cách 1: \(S=S_1+S_2\)").next_to(question, DOWN).to_edge(LEFT, buff=0.3)
        h1 = MathTex(r"(H_1):\begin{cases} y=\sqrt{x} \\ y=0 \\ x=0 \\ x=2 \end{cases}").next_to(t1, DOWN, aligned_edge=LEFT, buff=0.1)
        t2 = MathTex(r"S(H_1)=\int\limits_{0}^{2}\sqrt{x}\,\mathrm{d}x=1,885...\to A").next_to(h1, DOWN, aligned_edge=LEFT, buff=0.1)
        t3 = MathTex(r"\begin{cases} x\geqslant 2 \\ \sqrt{x}=x-2 \end{cases}").next_to(t2, DOWN, aligned_edge=LEFT, buff=0.1)
        t4 = MathTex(r"S(H_2)=\int\limits_{2}^{4}[\sqrt{x}-(x-2)]\,\mathrm{d}x=1,4477...\to B").next_to(t3, DOWN, aligned_edge=LEFT, buff=0.1)
        t5 = MathTex(r"\Rightarrow S=S(H_1)+S(H_2)=A+B=\frac{10}{3}").next_to(t4, DOWN, aligned_edge=LEFT, buff=0.1)
        t6 = Tex(r"Cách 2:").next_to(t5, DOWN, aligned_edge=LEFT, buff=0.1)
        t7 = MathTex(r"\begin{split} S&=S_{\mathit{OMN}}-S_{\triangle{KMN}} \\ &=\int\limits_{0}^{4}\sqrt{x}\,\mathrm{d}x-\frac{1}{2}\cdot 2 \cdot 2 = \frac{10}{3} \end{split}").next_to(t6, DOWN, aligned_edge=LEFT, buff=0.1)
